...
#add caves
#increase frame rate to 20-30 FPS
...
class game:
    ...
    def generateTerrainData(worldMaxHeight = 100, worldMinHeight = 100, worldWidth = 1000):
        ...
        for chunk in range(int(worldWidth)):
            ...
            #world does not obey min/max height boundaries
        return dataDict
...
while (game.running):
    ...
    if (game.stage == 'startscreen'):
        ...
        if (game.debugMode):
            ...
            playerPos = game.text('./utils/fonts/arial.ttf', 20, 'X:{}   Y:{}   MP:{}   CH:{}'.format(int(game.charPosition[0]), int(game.charPosition[1]), game.mousePos, centerHeight), (0, 0, 0))
